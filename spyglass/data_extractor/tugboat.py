...
from spyglass.data_extractor.excel_parser import ExcelParser
...
class TugboatPlugin(BaseDataSourcePlugin):
    ...
    def _get_ipmi_data(self):
        self.ipmi_data = self.excel_obj.get_ipmi_data()[0]
    ...
    def get_hosts(self, region, rack):
        hosts = []
        for host in self.ipmi_data.keys():
            hosts.append(
                {
                    'name': host,
                    'type': self.ipmi_data[host]['type'],
                    'host_profile': self.ipmi_data[host]['host_profile']
                }
            )
        return hosts
